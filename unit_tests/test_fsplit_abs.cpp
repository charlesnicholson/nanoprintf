...
TEST(fsplit_abs, Zero) {
    CHECK(npf_fsplit_abs(0.f, &i, &f, &f_neg_exp));
    CHECK_EQUAL(0, i);
    CHECK_EQUAL(0, f);
...
TEST(fsplit_abs, One) {
    CHECK(npf_fsplit_abs(1.f, &i, &f, &f_neg_exp));
    CHECK_EQUAL(1, i);
    CHECK_EQUAL(0, f);
    CHECK_EQUAL(0, f_neg_exp);
}
...
TEST(fsplit_abs, LargePositiveInteger) {
    CHECK(npf_fsplit_abs(123456.f, &i, &f, &f_neg_exp));
...
TEST(fsplit_abs, ReturnsZeroIfExponentTooLarge) {
    CHECK(npf_fsplit_abs(powf(2.0f, 63.f), &i, &f, &f_neg_exp));
    CHECK_EQUAL(0, npf_fsplit_abs(powf(2.0f, 64.f), &i, &f, &f_neg_exp));
}

// Fractional negative base 10 exponent (how many 0's between . and non-zero)

TEST(fsplit_abs, FracBase10NegExp_One) {
    CHECK(npf_fsplit_abs(0.03125f, &i, &f, &f_neg_exp));
    CHECK_EQUAL(1, f_neg_exp);
}

TEST(fsplit_abs, FracBase10NegExp_Two) {
    CHECK(npf_fsplit_abs(0.0078125f, &i, &f, &f_neg_exp));
    CHECK_EQUAL(2, f_neg_exp);
}

TEST(fsplit_abs, FracBase10NegExp_Three) {
    CHECK(npf_fsplit_abs(2.4414062E-4f, &i, &f, &f_neg_exp));
    CHECK_EQUAL(3, f_neg_exp);
}

TEST(fsplit_abs, FracBase10NegExp_Five) {
    CHECK(npf_fsplit_abs(3.8146973E-6f, &i, &f, &f_neg_exp));
...
TEST(fsplit_abs, 5) {
    CHECK(npf_fsplit_abs(1.5f, &i, &f, &f_neg_exp));
    CHECK_EQUAL(1, i);
    CHECK_EQUAL(5, f);
}

TEST(fsplit_abs, 625) {
    CHECK(npf_fsplit_abs(1.625f, &i, &f, &f_neg_exp));
    CHECK_EQUAL(1, i);
    CHECK_EQUAL(625, f);
}

TEST(fsplit_abs, 875) {
    CHECK(npf_fsplit_abs(1.875f, &i, &f, &f_neg_exp));
    CHECK_EQUAL(1, i);
    CHECK_EQUAL(875, f);
}

TEST(fsplit_abs, 9375) {
    CHECK(npf_fsplit_abs(1.9375f, &i, &f, &f_neg_exp));
    CHECK_EQUAL(1, i);
    CHECK_EQUAL(9375, f);
}

TEST(fsplit_abs, 96875) {
    CHECK(npf_fsplit_abs(1.96875f, &i, &f, &f_neg_exp));
    CHECK_EQUAL(1, i);
    CHECK_EQUAL(96875, f);
}

TEST(fsplit_abs, 984375) {
    CHECK(npf_fsplit_abs(1.984375f, &i, &f, &f_neg_exp));
    CHECK_EQUAL(1, i);
    CHECK_EQUAL(984375, f);
}

TEST(fsplit_abs, 9921875) {
    CHECK(npf_fsplit_abs(1.9921875f, &i, &f, &f_neg_exp));
    CHECK_EQUAL(1, i);
...
TEST(fsplit_abs, 9960938) {
    CHECK(npf_fsplit_abs(1.9960938f, &i, &f, &f_neg_exp));
    CHECK_EQUAL(1, i);
